...
INT8U rd64(INT8U* myRdyTbl, INT8U* myRdyGrp) {
    ...
    while (1) {
        ...
        rand_num = random(64);
        head = rand_num >> 3;
        // if the number is already exists, find other    (your code)
        if ((*myRdyGrp) & myMapTbl[head] != 0 && myRdyTbl[head] & myMapTbl[rand_num & 0x07] != 0) {
            continue;
        }
        // insert number into myRdyTbl & myRdyGrp    (your code)
        (*myRdyGrp) |= myMapTbl[head];
        myRdyTbl[head] |= myMapTbl[rand_num & 0x07];
    ...
    if (data == '1') {
        ...
        for (j = 0; j < 4; j++) {
            ...
            while (1) {
                ...
                // find min (of 4 random numbers)    (your code)
                tempY = myUnMapTbl[myRdyGrp];
                tempX = myUnMapTbl[myRdyTbl[tempY]];
                min = (tempY << 3) + tempX;
